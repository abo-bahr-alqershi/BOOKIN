...
abstract class UnitInSectionImagesRemoteDataSource {
  ...
  Future<List<SectionImageModel>> getImages(String unitInSectionId, {int? page, int? limit});
  Future<bool> updateImage(String imageId, Map<String, dynamic> data);
  Future<bool> deleteImage(String unitInSectionId, String imageId, {bool permanent});
...
class UnitInSectionImagesRemoteDataSourceImpl implements UnitInSectionImagesRemoteDataSource {
  final ApiClient apiClient;
  UnitInSectionImagesRemoteDataSourceImpl({required this.apiClient});

  static String _base(String unitInSectionId) => '/api/admin/unit-items/$unitInSectionId/images';
  ...
  Future<SectionImageModel> uploadImage({
    ...
    try {
      ...
      final response = await apiClient.post('${_base(unitInSectionId)}/upload', data: formData, options: Options(headers: {'Content-Type': 'multipart/form-data'}), onSendProgress: onSendProgress);
  ...
  @override
  Future<List<SectionImageModel>> getImages(String unitInSectionId, {int? page, int? limit}) async {
    try {
      final response = await apiClient.get(_base(unitInSectionId), queryParameters: { if (page != null) 'page': page, if (limit != null) 'limit': limit });
      final data = response.data is Map<String, dynamic> ? response.data : null;
      if (data != null) {
  ...
  @override
  Future<bool> deleteImage(String unitInSectionId, String imageId, {bool permanent = false}) async {
    try {
      final response = await apiClient.delete('${_base(unitInSectionId)}/$imageId', queryParameters: {'permanent': permanent});
  ...
  @override
  Future<bool> reorderImages(String unitInSectionId, List<String> imageIds) async {
    try {
      final response = await apiClient.post('${_base(unitInSectionId)}/reorder', data: {'imageIds': imageIds});
      return response.statusCode == 204 || (response.data is Map && response.data['success'] == true);
    } on DioException catch (e) {
  ...
  Future<bool> setAsPrimaryImage(String unitInSectionId, String imageId) async {
    try {
      final response = await apiClient.post('${_base(unitInSectionId)}/$imageId/set-primary');
      return response.statusCode == 204 || (response.data is Map && response.data['success'] == true);
    } on DioException catch (e) {
