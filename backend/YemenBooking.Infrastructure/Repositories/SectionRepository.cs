using Microsoft.EntityFrameworkCore;
using System.Linq;
using YemenBooking.Core.Entities;
using YemenBooking.Core.Enums;
using YemenBooking.Core.Interfaces.Repositories;
using YemenBooking.Infrastructure.Data.Context;

namespace YemenBooking.Infrastructure.Repositories
{
	public class SectionRepository : BaseRepository<Section>, ISectionRepository
	{
		public SectionRepository(YemenBookingDbContext context) : base(context) { }

		public async Task<Section> CreateAsync(Section section, CancellationToken cancellationToken = default)
		{
			await _dbSet.AddAsync(section, cancellationToken);
			await _context.SaveChangesAsync(cancellationToken);
			return section;
		}

		public async Task<Section> UpdateAsync(Section section, CancellationToken cancellationToken = default)
		{
			_dbSet.Update(section);
			await _context.SaveChangesAsync(cancellationToken);
			return section;
		}

		public async Task<bool> DeleteAsync(Guid sectionId, CancellationToken cancellationToken = default)
		{
			var entity = await _dbSet.Include(s => s.Items).FirstOrDefaultAsync(s => s.Id == sectionId, cancellationToken);
			if (entity == null) return false;
			_context.RemoveRange(entity.Items);
			_dbSet.Remove(entity);
			await _context.SaveChangesAsync(cancellationToken);
			return true;
		}

		public async Task<(IEnumerable<Section> Items, int TotalCount)> GetPagedAsync(int pageNumber, int pageSize, SectionTarget? target, SectionType? type, CancellationToken cancellationToken = default)
		{
			var query = _dbSet.AsQueryable();
			if (target.HasValue) query = query.Where(s => s.Target == target);
			if (type.HasValue) query = query.Where(s => s.Type == type);
			var total = await query.CountAsync(cancellationToken);
			var items = await query
				.OrderBy(s => s.DisplayOrder)
				.Skip((pageNumber - 1) * pageSize)
				.Take(pageSize)
				.ToListAsync(cancellationToken);
			return (items, total);
		}

		public async Task<IEnumerable<SectionItem>> GetItemsAsync(Guid sectionId, CancellationToken cancellationToken = default)
		{
			return await _context.SectionItems
				.Where(i => i.SectionId == sectionId)
				.OrderBy(i => i.SortOrder)
				.ToListAsync(cancellationToken);
		}

		public async Task AssignPropertiesAsync(Guid sectionId, IEnumerable<Guid> propertyIds, CancellationToken cancellationToken = default)
		{
			var existing = await _context.SectionItems.Where(i => i.SectionId == sectionId).ToListAsync(cancellationToken);
			_context.SectionItems.RemoveRange(existing);
			var toAdd = propertyIds.Distinct().Select(pid => new SectionItem { SectionId = sectionId, PropertyId = pid }).ToList();
			await _context.SectionItems.AddRangeAsync(toAdd, cancellationToken);
			await _context.SaveChangesAsync(cancellationToken);
		}

		public async Task AssignUnitsAsync(Guid sectionId, IEnumerable<Guid> unitIds, CancellationToken cancellationToken = default)
		{
			var existing = await _context.SectionItems.Where(i => i.SectionId == sectionId).ToListAsync(cancellationToken);
			_context.SectionItems.RemoveRange(existing);
			var toAdd = unitIds.Distinct().Select(uid => new SectionItem { SectionId = sectionId, UnitId = uid }).ToList();
			await _context.SectionItems.AddRangeAsync(toAdd, cancellationToken);
			await _context.SaveChangesAsync(cancellationToken);
		}

		public async Task AddPropertiesAsync(Guid sectionId, IEnumerable<Guid> propertyIds, CancellationToken cancellationToken = default)
		{
			var existingKeys = await _context.SectionItems.Where(i => i.SectionId == sectionId && i.PropertyId != null).Select(i => i.PropertyId!.Value).ToListAsync(cancellationToken);
			var toAdd = propertyIds.Distinct().Except(existingKeys).Select(pid => new SectionItem { SectionId = sectionId, PropertyId = pid }).ToList();
			if (toAdd.Count > 0)
			{
				await _context.SectionItems.AddRangeAsync(toAdd, cancellationToken);
				await _context.SaveChangesAsync(cancellationToken);
			}
		}

		public async Task AddUnitsAsync(Guid sectionId, IEnumerable<Guid> unitIds, CancellationToken cancellationToken = default)
		{
			var existingKeys = await _context.SectionItems.Where(i => i.SectionId == sectionId && i.UnitId != null).Select(i => i.UnitId!.Value).ToListAsync(cancellationToken);
			var toAdd = unitIds.Distinct().Except(existingKeys).Select(uid => new SectionItem { SectionId = sectionId, UnitId = uid }).ToList();
			if (toAdd.Count > 0)
			{
				await _context.SectionItems.AddRangeAsync(toAdd, cancellationToken);
				await _context.SaveChangesAsync(cancellationToken);
			}
		}

		public async Task RemoveItemAsync(Guid sectionId, Guid itemId, CancellationToken cancellationToken = default)
		{
			var item = await _context.SectionItems.FirstOrDefaultAsync(i => i.SectionId == sectionId && i.Id == itemId, cancellationToken);
			if (item != null)
			{
				_context.SectionItems.Remove(item);
				await _context.SaveChangesAsync(cancellationToken);
			}
		}

		public async Task ReorderItemsAsync(Guid sectionId, IReadOnlyList<(Guid ItemId, int SortOrder)> orders, CancellationToken cancellationToken = default)
		{
			var itemIds = orders.Select(o => o.ItemId).ToList();
			var items = await _context.SectionItems.Where(i => i.SectionId == sectionId && itemIds.Contains(i.Id)).ToListAsync(cancellationToken);
			foreach (var item in items)
			{
				var ord = orders.First(o => o.ItemId == item.Id).SortOrder;
				item.SortOrder = ord;
			}
			await _context.SaveChangesAsync(cancellationToken);
		}
	}
}